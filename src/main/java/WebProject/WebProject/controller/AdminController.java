...
public class AdminController {
    ...
    public String SendMessage(Model model, @ModelAttribute("message") String message,
        ...
        Mail mail = new Mail();
        mail.setMailFrom("hoang1521065@huce.edu.vn");
        mail.setMailTo(email);
        mail.setMailSubject("This is message from Fresh Life Jewelry.");
    ...
    public String DashboardWalletView(Model model) {
        ...
        if (admin == null) {
            ...
            List<Order> listOrder = orderService.findAll();
            List<Order> listPaymentWithMomo = orderService.findAllByPayment_Method("Payment with VNPay");
            ...
            List<Order> listRecentMomo = orderService.findTop5OrderByPaymentMethod("Payment with VNPay");
    ...
    public String DashboardMyProductEditHandel(Model model,
                                               ...
                                               @ModelAttribute("isActive") Integer isActive,
...
//				Product newPro = new Product();
                ...
                product.setQuantity(Long.parseLong(availability));
...
//			int category_selected = 0;
    ...
    public String DashboardUsersView(Model model) {
        User admin = (User) session.getAttribute("admin");
        if (admin == null) {
            return "redirect:/signin-admin";
        } else {
    ...
    public String DashboardUsersEditView(@PathVariable String id, Model model) {
        User admin = (User) session.getAttribute("admin");
        if (admin == null) {
            return "redirect:/signin-admin";
        } else {
    ...
    public String DashboardUserEditHandle(Model model, @ModelAttribute("user_id") String user_id,
                                          @ModelAttribute("user_Name") String user_Name,
                                          @ModelAttribute("phone_Number") String phone_Number,
        ...
        if (admin == null) {
            ...
            long existingUsersWithEmailCount = userService.countByEmail(email);

            if (existingUsersWithEmailCount > 0 && !user.getEmail().equals(email)) {
                // Email đã tồn tại và khác với email hiện tại của người dùng
                session.setAttribute("editUser", "emailExists");
            ...
            session.setAttribute("editUser", "editUserSuccess");
            return "redirect:/dashboard-users/edit/" + user_id;
        }
    }
    ...
    public String DashboardUsersPageView(@PathVariable int page, Model model) {
        User admin = (User) session.getAttribute("admin");
        if (admin == null) {
            return "redirect:/signin-admin";
        } else {
    ...
    public String deleteUser(@PathVariable("id") String user_id, Model model) {
        User admin = (User) session.getAttribute("admin");
        if (admin == null) {
            return "redirect:/signin-admin";
        } else {
    ...
    public String deleteProducts(@PathVariable("id") int product_id, Model model) {
        User admin = (User) session.getAttribute("admin");
        if (admin == null) {
            return "redirect:/signin-admin";
        } else {
    ...
    public String DashboardUsersSearch(@ModelAttribute("search-input") String search_input,
                                       @ModelAttribute("status") int status, Model model) {
        User admin = (User) session.getAttribute("admin");
        if (admin == null) {
            return "redirect:/signin-admin";
        } else {
            ...
            Pageable pageable = PageRequest.of(0, 5);
            if(status == 2) {
                pageUser = userService.findByEmailContaining(search_input, pageable);
            }
            else {
                pageUser = userService.findByEmailAndStatusContaining(search_input, status,
                        pageable);
            }
            model.addAttribute("pageUser", pageUser);
            model.addAttribute("search_dashboard", "search_dashboard");
    ...
    @GetMapping("/dashboard-users/search/{page}")
    public String DashboardUsersearchPage(@PathVariable int page, Model model) {
        User admin = (User) session.getAttribute("admin");
        if (admin == null) {
            return "redirect:/signin-admin";
        } else {
